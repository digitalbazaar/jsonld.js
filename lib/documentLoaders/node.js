...
module.exports = ({
  ...
  async function loadDocument(url, redirects) {
    ...
    // handle Link Header
    if(link &&
      res.headers.get('content-type') !== 'application/ld+json') {
      ...
      if(alternate &&
        ...
        // FIXME not sure we can do this anymore
...
// FIXME turn strictSSL into an httpsAgent
async function _fetch({url, headers, strictSSL}) {
  try {
    const res = await httpClient.get(
      url, {headers, strictSSL, redirect: 'manual'});
