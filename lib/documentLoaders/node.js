...
module.exports = options => {
  ...
  const strictSSL = ('strictSSL' in options) ? options.strictSSL : true;
  const maxRedirects = ('maxRedirects' in options) ? options.maxRedirects : -1;
  const request = ('request' in options) ? options.request : require('request');
  const http = require('http');
  // TODO: disable cache until HTTP caching implemented
  //const cache = new jsonld.DocumentCache();
