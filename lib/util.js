...
api.hasValue = (subject, property, value) => {
  return api.hasProperty(subject, property) &&
    _hasValue(subject, property, value);
};
...
api.compareValues = (v1, v2) => {
  ...
  if(types.isObject(v1) && types.isObject(v2)) {
    const id1 = v1['@id'];
    const id2 = v2['@id'];
    return id1 !== undefined && v1['@id'] === v2['@id'];
  }
...
function _addValue(subject, property, value, options) {
  if(options.valueIsArray) {
    ...
    _addArrayValue(subject, property, value, options);
...
// internal helper for `api.addValue`
function _addArrayValue(subject, property, value, options) {
  if(value.length === 0 && options.propertyIsArray &&
    !subject.hasOwnProperty(property)) {
    subject[property] = [];
  }
  for(let i = 0; i < value.length; ++i) {
    _addValue(subject, property, value[i], options);
  }
}
...
function _addToExistingProperty(subject, property, value, options) {
  ...
  if(!types.isArray(subject[property]) &&
    ...
    if(hasValue) {
      ...
      subject[property] = [value, subject[property]];
    } else {
...
// internal helper that assumes `subject` has `property`
function _hasValue(subject, property, value) {
  let val = subject[property];
  const isList = graphTypes.isList(val);
  const isArray = types.isArray(val);
  if(isArray || isList) {
    if(isList) {
      val = val['@list'];
    }
