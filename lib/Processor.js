      ...
      if (_isObject(element)) {
        ...
        for (var ki = 0; ki < keys.length; ++ki) {
            ...
            if (_isString(expandedValue)) {
              ...
              for (var vi = 0; vi < expandedValue.length; ++vi) {
                compactedValue.push(_compactIri(
                  activeCtx, expandedValue[vi], null, {
                    vocab: true
                  }));
              }
            }
          ...
          if (expandedProperty === '@index') {
            ...
            // use keyword alias and add value
            var alias = _compactIri(activeCtx, expandedProperty);
            addValue(rval, alias, expandedValue);
            continue;
          }
          ...
          if (expandedValue.length === 0) {
            var itemActiveProperty = _compactIri(
              activeCtx, expandedProperty, expandedValue, {
                vocab: true
              },
              insideReverse);
            addValue(
              rval, itemActiveProperty, expandedValue, {
          ...
          for (var vi = 0; vi < expandedValue.length; ++vi) {
            ...
            var itemActiveProperty = _compactIri(
              activeCtx, expandedProperty, expandedItem, {
                vocab: true
              },
              insideReverse);
            var container = getContextValue(
            ...
            if (container === '@language' || container === '@index') {
              ...
              addValue(mapObject, expandedItem[container], compactedItem);
            } else {
...
Processor.prototype.expand = function(
      ...
      if (!_isArray(element) && !_isObject(element)) {
        ...
        if (!insideList && (activeProperty === null ||
          _expandIri(activeCtx, activeProperty, {
            vocab: true
          }) === '@graph')) {
          return null;
        }
      ...
      for (var ki = 0; ki < keys.length; ++ki) {
        ...
        var container = getContextValue(activeCtx, key, '@container');
        ...
        var useArray = ['@index', '@id', '@type', '@value', '@language'].indexOf(
          expandedProperty) === -1;
        addValue(
          rval, expandedProperty, expandedValue, {
            propertyIsArray: useArray
          });
      }
...
Processor.prototype.frame = function(input, _declobberedframe, options) {
      ...
      var issuer = new _declobberedIdentifierIssuer('_:b');
      _createNodeMap(input, state.graphs, '@merged', issuer);
      state.subjects = state.graphs['@merged'];

      // frame the subjects
      var framed = [];
      _frame(state, Object.keys(state.subjects).sort(), _declobberedframe, framed, null);
      return framed;
    };
...
Processor.prototype.processContext = function(activeCtx, localCtx, options) {
      ...
      for (var i = 0; i < ctxs.length; ++i) {
        ...
        if (cache.activeCtx) {
          var cached = cache.activeCtx.get(activeCtx, ctx);
          if (cached) {
        ...
        // cache result
        if (cache.activeCtx) {
          cache.activeCtx.set(activeCtx, ctx, rval);
        }
      }
